...
namespace Universe
{
	...
	/// </summary>
	[Serializable]
	public class UniVec3 : IEquatable<UniVec3>
	{
		public Long3   Units;
		...
		public Double3 ToLocalDouble(in UniVec3 observer)
		{
			...
			return new Double3(
				(delta.Units.X * delta.Scale) + delta.Offset.X,
				(delta.Units.Y * delta.Scale) + delta.Offset.Y,
				(delta.Units.Z * delta.Scale) + delta.Offset.Z
			);
		}
		...
		public static UniVec3 Max(UniVec3 a, UniVec3 b)
		{
			...
			var (ax, ay, az) = a.ToDouble();
			var (bx, by, bz) = b.ToDouble();
			return FromDouble(Math.Max(ax, bx), Math.Max(ay, by), Math.Max(az, bz), a.Scale);
		}
		...
		{
			if (Math.Abs(b.Scale - scale) > EPSILON)
				b = b.Convert(scale);
		}
		...
		public bool Equals(UniVec3 other) =>
			Units == other.Units &&
			Offset == other.Offset &&
			Math.Abs(Scale - other.Scale) < EPSILON;
